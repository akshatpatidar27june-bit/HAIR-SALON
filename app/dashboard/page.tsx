...
export default function Dashboard() {
  ...
  if (loading) return <div className="grid min-h-screen place-items-center bg-[#1A1423]">Loading salon analytics…</div>;
  if (error) return <div className="grid min-h-screen place-items-center bg-[#1A1423]"><div className="rounded-3xl bg-white p-8 text-center"><h1 className="text-xl font-bold">Owner dashboard</h1><p className="mt-2 text-sm text-red-600">{error}</p><button onClick={() => load(outletId)} className="mt-5 rounded-xl bg-[#1A1423] px-4 py-2 text-sm text-white">Try again</button></div></div>;
  const outlet = outlets.find(o => o.id === outletId);

  return <div className="min-h-screen bg-[#1A1423] text-[#1A1423]">
    <aside className="fixed inset-y-0 left-0 hidden w-72 bg-[#1A1423] p-5 text-white lg:block"><nav className="mt-16 space-y-1">{nav.map(([x, I, path]: any) => <button key={x} onClick={() => router.push(path)} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-white/70 hover:bg-white/10"><I size={18} />{x}</button>)}</nav><button onClick={async () => { await supabase.auth.signOut(); router.replace('/signin'); }} className="absolute bottom-6 left-5 flex items-center gap-3 px-4 py-3 text-sm text-white/60"><LogOut size={18} />Sign out</button></aside>
    <div className={`fixed inset-0 z-50 lg:hidden ${mobileOpen ? '' : 'pointer-events-none'}`}><div onClick={() => setMobileOpen(false)} className={`absolute inset-0 bg-black/40 transition-opacity ${mobileOpen ? 'opacity-100' : 'opacity-0'}`} /><aside className={`absolute inset-y-0 left-0 w-72 bg-[#1A1423] p-5 text-white shadow-2xl transition-transform ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}><div className="flex justify-end"><button onClick={() => setMobileOpen(false)}><X /></button></div><nav className="mt-5 space-y-1">{nav.map(([x, I, path]: any) => <button key={x} onClick={() => { setMobileOpen(false); router.push(path); }} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-white/70 hover:bg-white/10"><I size={18} />{x}</button>)}</nav></aside></div>

    <main className="lg:pl-72"><header className="flex items-center justify-between border-b border-[#3A3047] bg-white/80 px-5 py-4"><button onClick={() => setMobileOpen(true)} className="rounded-xl border border-[#3A3047] p-2 lg:hidden"><Menu size={20} /></button><div className="ml-auto flex items-center gap-3"><button title="Refresh analytics" onClick={() => load(outletId, true)} className="rounded-xl border border-[#3A3047] bg-white p-2"><RefreshCw size={17} className={refreshing ? 'animate-spin' : ''} /></button><div className="rounded-2xl border border-[#3A3047] bg-white px-4 py-2 text-right"><b className="text-xs">{user?.name || 'Owner'}</b><div className="text-[10px] text-[#B9B1C4] capitalize">{user?.role || 'owner'}</div></div></div></header>

      <section className="mx-auto max-w-[1550px] p-5 md:p-8">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.25em] text-[#FFC914]">Owner intelligence</p><h1 className="mt-2 text-4xl font-bold">Good morning, {user?.name || 'Owner'}.</h1><p className="mt-2 text-sm text-[#B9B1C4]">Live business analytics • {outlet?.name || 'Outlet'} • {tx.length.toLocaleString('en-IN')} transactions loaded</p></div><div className="flex flex-wrap items-center gap-2"><div className="flex rounded-2xl border border-[#3A3047] bg-white p-1">{(['7d', '30d', '90d', 'year'] as RangeKey[]).map(r => <button key={r} onClick={() => setRange(r)} className={`rounded-xl px-3 py-2 text-xs font-bold ${range === r ? 'bg-[#1A1423] text-white' : 'text-[#B9B1C4]'}`}>{r === 'year' ? 'This year' : r.replace('d', ' days')}</button>)}</div><select value={outletId} onChange={e => { localStorage.setItem('hair_salon_outlet', e.target.value); load(e.target.value); }} className="rounded-2xl border-2 border-[#FFC914] bg-white px-4 py-3 text-sm font-semibold shadow-sm">{outlets.map(o => <option key={o.id} value={o.id}>{o.name} — {o.city}</option>)}</select></div></div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_.55fr]"><section className="rounded-3xl border border-[#3A3047] bg-white p-6"><div className="flex items-start justify-between"><div><h2 className="font-bold">Revenue trend</h2><p className="mt-1 text-xs text-[#B9B1C4]">Actual completed transactions for the selected period</p></div><div className="text-right"><div className="text-xs text-[#B9B1C4]">Cash + UPI</div><b>{money(cash + upi)}</b></div></div><LineChart data={dailySales} /></section><div className="space-y-6"><section className="rounded-3xl bg-[#1A1423] p-6 text-white"><p className="text-xs uppercase tracking-[.2em] text-white/40">This month</p><div className="mt-2 text-4xl font-bold">{money(sum(month))}</div><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-white" style={{ width: `${monthProgress}%` }} /></div><div className="mt-2 flex justify-between text-xs text-white/50"><span>{monthlyTarget ? `${monthProgress.toFixed(0)}% of historical monthly average` : 'Building baseline'}</span><span>{money(monthlyTarget)}</span></div></section><section className="rounded-3xl border border-[#3A3047] bg-white p-6"><h2 className="font-bold">Payment mix</h2><div className="mt-5 flex items-center gap-5"><Donut cash={cash} upi={upi} total={revenue} /><div className="space-y-3 text-sm"><div><span className="inline-block h-2 w-2 rounded-full bg-[#1A1423] mr-2" />Cash <b>{money(cash)}</b></div><div><span className="inline-block h-2 w-2 rounded-full bg-[#FFC914] mr-2" />UPI <b>{money(upi)}</b></div></div></div></section></div></div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_1fr_.75fr]"><section className="rounded-3xl border border-[#3A3047] bg-white p-6"><div className="flex items-center gap-2"><AlertTriangle size={18} /><h2 className="font-bold">Attention radar</h2></div><p className="mt-1 text-xs text-[#B9B1C4]">Customers past their normal return interval</p><div className="mt-5 space-y-3">{attentionCustomers.length ? attentionCustomers.map(c => <CustomerRow key={c.id} c={c} tone="attention" />) : <Empty text="No customers currently flagged by the activity rules." />}</div></section><section className="rounded-3xl border border-[#3A3047] bg-white p-6"><div className="flex items-center gap-2"><Target size={18} /><h2 className="font-bold">High-value customers</h2></div><p className="mt-1 text-xs text-[#B9B1C4]">Top customers by lifetime recorded spend</p><div className="mt-5 space-y-3">{highValueCustomers.length ? highValueCustomers.map(c => <CustomerRow key={c.id} c={c} tone="value" />) : <Empty text="Not enough customer history yet." />}</div></section><section className="rounded-3xl bg-[#102a35] p-6"><div className="flex items-center gap-2"><Sparkles size={18} /><h2 className="font-bold">Daily briefing</h2></div><div className="mt-5 space-y-4 text-sm"><Brief icon={<IndianRupee size={15} />} text={`${money(sum(today))} sales from ${today.length} completed services today.`} /><Brief icon={<Users size={15} />} text={`${uniqueCustomers} unique customers are in the selected period.`} /><Brief icon={<Clock3 size={15} />} text={peakHours.length ? `Busiest recorded hour: ${formatHour(peakHours[0].hour)} (${peakHours[0].count} services).` : 'No peak-hour data yet.'} /><Brief icon={<AlertTriangle size={15} />} text={`${attentionCustomers.length} customers currently need attention based on their own visit history.`} /></div></section></div>

        <div className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_.7fr]"><section className="rounded-3xl border border-[#3A3047] bg-white p-6"><div className="flex items-start justify-between"><div><h2 className="font-bold">Customer intelligence</h2><p className="mt-1 text-xs text-[#B9B1C4]">Lifetime behaviour calculated from recorded transactions</p></div><button onClick={() => router.push('/dashboard/customers')} className="text-xs font-bold text-[#FFC914]">Open customers →</button></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead><tr>{['Customer', 'Visits', 'Lifetime spend', 'Avg bill', 'Last visit', 'Signal'].map(h => <th key={h} className="pb-3 text-xs text-[#B9B1C4]">{h}</th>)}</tr></thead><tbody>{customerInsights.slice(0, 12).map(c => <tr key={c.id} className="border-t border-[#18343f]"><td className="py-4 font-semibold">{c.name}</td><td>{c.visits}</td><td className="font-semibold">{money(c.spend)}</td><td>{money(c.avgBill)}</td><td>{formatRelative(c.daysSince)}</td><td><Badge status={c.status} /></td></tr>)}</tbody></table>{!customerInsights.length && <Empty text="Customer analytics will appear after transactions are recorded." />}</div></section><section className="rounded-3xl border border-[#3A3047] bg-white p-6"><h2 className="font-bold">Peak-time intelligence</h2><p className="mt-1 text-xs text-[#B9B1C4]">Highest appointment volume in the selected period</p><div className="mt-5 space-y-4">{peakHours.length ? peakHours.map(h => <div key={h.hour}><div className="mb-1 flex justify-between text-xs"><b>{formatHour(h.hour)}</b><span>{h.count} services • {money(h.revenue)}</span></div><div className="h-2 rounded-full bg-[#18343f]"><div className="h-full rounded-full bg-[#1A1423]" style={{ width: `${Math.max(8, (h.count / peakHours[0].count) * 100)}%` }} /></div></div>) : <Empty text="No appointment data for this period." />}</div><div className="mt-7 border-t border-[#18343f] pt-5"><h3 className="text-sm font-bold">Customer segments</h3><div className="mt-4 grid grid-cols-2 gap-3"><Mini title="High value" value={highValueCustomers.length} /><Mini title="Needs attention" value={attentionCustomers.length} /><Mini title="New" value={newCustomers.length} /><Mini title="Regular" value={activeCustomers.length} /></div></div></section></div>

        <div className="mt-6 rounded-2xl border border-dashed border-[#24505d] bg-white/50 p-4 text-xs text-[#B9B1C4]"><b>Data note:</b> this dashboard uses the salon's recorded transactions. Cancellation/no-show analytics will become available once the database stores appointment statuses; the current schema only records completed services/payments, so the dashboard does not invent those numbers.</div>
      </section>
    </main>
  </div>;
}

function Metric({ title, value, sub, icon, trend }: { title: string; value: string; sub: string; icon: React.ReactNode; trend?: number | null }) { return <div className="rounded-3xl border border-[#3A3047] bg-white p-5 shadow-sm"><div className="flex justify-between text-sm text-[#B9B1C4]">{title}<span>{icon}</span></div><div className="mt-6 text-3xl font-bold">{value}</div><div className={`mt-2 flex items-center gap-1 text-xs ${trend !== undefined && trend !== null ? (trend >= 0 ? 'text-[#54734e]' : 'text-[#a45b4e]') : 'text-[#FFC914]'}`}>{trend !== undefined && trend !== null && (trend >= 0 ? <TrendingUp size={13} /> : <TrendingDown size={13} />)}{sub}</div></div>; }

function LineChart({ data }: { data: { label: string; value: number }[] }) { const max = Math.max(...data.map(x => x.value), 1); const w = 760, h = 240, pad = 28; const points = data.map((d, i) => `${pad + (i * (w - pad * 2)) / Math.max(data.length - 1, 1)},${h - pad - (d.value / max) * (h - pad * 2)}`).join(' '); return <div className="mt-5 overflow-hidden"><svg viewBox={`0 0 ${w} ${h}`} className="h-[260px] w-full" preserveAspectRatio="none"><line x1={pad} x2={w - pad} y1={h - pad} y2={h - pad} stroke="#3A3047" /><polyline fill="none" stroke="#1A1423" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" points={points} />{data.map((d, i) => { const x = pad + (i * (w - pad * 2)) / Math.max(data.length - 1, 1); const y = h - pad - (d.value / max) * (h - pad * 2); return <circle key={i} cx={x} cy={y} r="4" fill="#FFC914" />; })}</svg><div className="flex justify-between gap-2 text-[10px] text-[#B9B1C4]">{data.filter((_, i) => i === 0 || i === data.length - 1 || i === Math.floor(data.length / 2)).map((d, i) => <span key={i}>{d.label}</span>)}</div></div>; }

function BarPanel({ title, subtitle, data, money: moneyMode }: { title: string; subtitle: string; data: { name: string; value: number; count: number }[]; money?: boolean }) { const max = Math.max(...data.map(x => x.value), 1); return <section className="rounded-3xl border border-[#3A3047] bg-white p-6"><h2 className="font-bold">{title}</h2><p className="mt-1 text-xs text-[#B9B1C4]">{subtitle}</p><div className="mt-6 space-y-4">{data.length ? data.map((x, i) => <div key={x.name}><div className="mb-1 flex justify-between gap-3 text-xs"><b className="truncate">{i + 1}. {x.name}</b><span>{moneyMode ? money(x.value) : x.value.toLocaleString('en-IN')} • {x.count} visits</span></div><div className="h-3 rounded-full bg-[#18343f]"><div className="h-full rounded-full bg-[#1A1423]" style={{ width: `${Math.max(3, x.value / max * 100)}%` }} /></div></div>) : <Empty text="No data in this period." />}</div></section>; }

function CustomerRow({ c, tone }: { c: CustomerInsight; tone: 'attention' | 'value' }) { return <div className="flex items-center gap-3 rounded-2xl border border-[#18343f] p-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#12313b] text-xs font-bold">{c.name[0]?.toUpperCase()}</div><div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><b className="truncate text-sm">{c.name}</b><span className="text-xs font-bold">{money(c.spend)}</span></div><div className="mt-1 text-[11px] text-[#B9B1C4]">{c.visits} visits • avg {money(c.avgBill)} • {tone === 'attention' ? `${c.daysSince} days since visit` : 'lifetime recorded spend'}</div></div></div>; }
...
function Mini({ title, value }: { title: string; value: number }) { return <div className="rounded-2xl bg-[#241C2F] p-3"><div className="text-xl font-bold">{value}</div><div className="mt-1 text-[10px] text-[#B9B1C4]">{title}</div></div>; }
function Empty({ text }: { text: string }) { return <p className="py-8 text-center text-xs text-[#B9B1C4]">{text}</p>; }
function Donut({ cash, upi, total }: { cash: number; upi: number; total: number }) { const cashPct = total ? cash / total * 100 : 50; return <div className="grid h-24 w-24 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(#1A1423 0 ${cashPct}%, #FFC914 ${cashPct}% 100%)` }}><div className="grid h-14 w-14 place-items-center rounded-full bg-white text-center"><b className="text-xs">{total ? '100%' : '—'}</b><span className="text-[8px] text-[#B9B1C4]">mix</span></div></div>; }
